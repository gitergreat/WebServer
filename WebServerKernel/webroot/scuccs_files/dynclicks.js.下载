...
function _showDynClicks(clicktype, owner, clickid)
{
    try
    {
        ...
        if(owner=="1420436181"){
        	xmlhttp.onreadystatechange  = function(){_onShowDynClicks1(xmlhttp, dynobj);};
        }else{
...
function _onShowDynClicks1(xmlhttp, dynobj)
{
    if(xmlhttp.readyState == 4 && xmlhttp.status == 200)
    {
        try
        {
        	 var a = parseInt(xmlhttp.responseText)+1000;
            document.getElementById(dynobj).innerHTML = a.toString();
            //针对2个内容重叠而且 id 相同时的处理方法
            var imgobjs = document.getElementsByName(dynobj);
            if(imgobjs.length>0)
            { 
                     ...
                     imgobjs[1].innerHTML = a.toString();
            }            
        }
        catch(e)
        {
